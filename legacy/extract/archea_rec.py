...
#
# ============================================================================
# archea_rec.py - 考古线图轮廓分类识别：比例尺 / 数字序号 / 说明文字。
#
# 任务定位（archea_seg.py 的下游独立识别模块，供其它应用方调用）：
#   本模块不再自行做轮廓分割——调用方须先经 archea_seg 完成分割，并把
#   其结果传入 recognize(image, kept)：
#     kept 推荐传 archea_seg.extract_contours() 的保留轮廓结果（全量组件，
#       含文字小组件；OTSU→膨胀→findContours→孔洞/内含剔除已由其完成）；
#     亦兼容 ① 其三元组返回值 (kept, removed, binary)、
#           ② ArcheaSeg().segment() 结果 dict（由 instances.contour_poly
#             重建；注意 SAM 相交合并吸收掉的小组件不会出现，仅作兼容入口）。
#   收到轮廓后做几何 + OCR 分类：
#     1) 比例尺识别：比例尺组 = 图形化尺体（细长带刻度齿）+ 起点"0"
#        + "x厘米"值文本，三者组合成组（如 "0——4厘米"）；比例尺左侧的
#        前缀数字（如 "1、2. 0——4厘米" 中的 "1、2."）不属于比例尺，
#        作为 数字序号 在 serials 中独立分割识别输出。横排/竖排（旋转90°）
#        均支持；无尺体但含单位词的单元按"纯文字尺寸标注"单独成组（docx 规范）。
#     2) 非比例尺区域：纯数字→器物序号 serial（带 scale_prefix 标记区分
#        比例尺左侧前缀数字与正文序号；serial_set 只计正文序号）；
#        digits_only=True（默认）时只识别数字序号，文字/字母读值归
#        others(text)，texts 恒空（CLI --all-text 可恢复）；
#        读不出→未定 other（bbox 仍保留供下游/人工复核）。
#     3) 连接符判别：读作 '1' 的细长竖条与视图连接线形态相同（实测本
#        数据集数字 '1' 常无斜帽），按上下文判别——图形内部或高度超过
#        本图其他数字中位高 1.5 倍 → others(connector) 做出标识，防混
#        入序号；其余保守判数字 '1'。
#     3) 噪声碎片抑制（OCR 后联合判定，归入 others，不污染 serials/texts）：
#        scale_zone 与已证认尺体 bbox 相交的未吸收单元（起点0误读/刻度残笔，
#        含起点0贴尺邻接判定与"1、2.0"粘连切前缀）；
#        fig_zone 图形内/贴边碎片（阴影带/虚线刻度/剖面线；1-2 位纯数字且
#        h>=12 的序号形态豁免——不用 conf 门槛，rapid 小字置信度普遍偏低）；
#        tiny 微小噪点；sliver 低置信细长条。
#
# 识别后端（rec-only，纯本地）：
#   backend="auto"（默认，等价 "paddle"）: paddleocr TextRecognition，
#                           显式锁定 PP-OCRv6_medium_rec（FLAGS_use_mkldnn=0 双保险）。
#   rapidocr_onnxruntime 已停用：其内置 PP-OCRv4 识别效果低于 PP-OCRv6，
#                           不再作为降级备选。
#   "none"（无 OCR）同样停用：无读值时序号/比例尺识别必然缺失，属无效识别。
#   报警策略：backend 非 auto/paddle（如 none/rapid）、paddleocr 初始化失败
#                           或识别调用失败 => 立即抛异常报警（消息带 ⚠ 报警
#                           前缀），不静默降级、不产出空读值结果。
#   可用 kwarg backend= / 环境变量 ARCHEA_OCR_BACKEND 指定（仅 auto|paddle）。
#
# 输入格式（recognize(image) 的 image 参数）：与 archea_seg 一致，
#   文件路径 / bytes / BGR ndarray。
#
# 输出 dict：
#   {
...
#       {
#         "id": int,
#         "kind": "bar"|"text",                    # bar=图形尺体组 text=纯文字尺寸标注
#         "orientation": "h"|"v",                  # 横排/竖排
#         "bbox": [x0,y0,x1,y1],                   # 组包围盒（尺体+起点0+值文本）
#         "bar_bbox": [..]|None,                   # 尺体组件 bbox（kind=text 时 None）
#         "text": "0——4厘米",                       # 尺主体读值（band OCR 解析）
#         "raw_text": "...",                       # band OCR 原始读值（可能含前缀粘连）
#         "value": 4.0, "unit": "厘米",             # 白名单解析结果（可为 None）
#         "conf": float,                           # 成员最大置信度
#         "verified": bool,                        # 有 0/单位词证据或 band 解析成功
#         "ticks": int,                            # 尺体刻度齿列数
#         "members": [                             # 组成员（仅尺体/起点0/值文本）
#           {"role":"bar|zero|value","bbox":[..],"text":..,"conf":..}
#         ],
#         "crop_base64": str|None,                 # 尺行裁片（band，VL/复核输入）
#         "contour_poly": [[x,y],...]|None,        # 尺体轮廓简化多边形
#       }, ...
#     ],
#     "serials": [ {"id","bbox","text","no","conf","rotated","scale_prefix"} ],
#                  # 器物序号；scale_prefix=True 表示比例尺左侧前缀数字
#                  # （"1、2."/"1~2" 等），用于区分正文中的序号；
#                  # serial_set 集合校验只计正文序号（排除 scale_prefix）。
#                  # 比例尺左侧的前缀数字不归入比例尺，按 数字序号 在
#                  # serials 中独立输出（docx 序号硬性匹配用）。
#     "texts":   [ {"id","bbox","text","conf","rotated","relic"} ],# 说明文字
#                  # relic=True 表示读出器物号形态（M4:6，全局编号，非图内序号）
#                  # digits_only=True（默认）时恒为空，相关读值归 others(text)
#     "figures": [ {"bbox","area"} ],              # 图形大轮廓（器物线图等，未 OCR）
#     "others":  [ {"bbox","reason","text","conf"} ],  # 未定噪声碎片（OCR 后联合判定）：
#                  # reason ∈ unreadable（读不出）/ scale_zone（与已证认尺体相交
#                  # 或起点0贴尺的碎片）/ fig_zone（图形内/贴边碎片）/ tiny（微小
#                  # 噪点）/ sliver（低置信细长条）/ lowconf（尺寸门卫拒收）/
#                  # line（被降级的长线候选）
#     "serial_set": {"nums","missing","dups","suspect"},           # 序号集合校验
#     "stats": {"n_contours","n_figures","n_bars","n_scales",
#               "n_serials","n_texts","n_others","latency_s"}
#   }
#
# 坐标与阅读序：所有 bbox 为原图像素坐标 [x0,y0,x1,y1]；列表按阅读序
# （上->下、左->右）排列。
#
# 用法（应用方：先 archea_seg 分割，再交本模块识别）：
#   from archea_seg import extract_contours, load_image   # 分割阶段（调用方负责）
#   from archea_rec import ArcheaRec
#   rec = ArcheaRec()                                # backend 仅允许 auto|paddle，异常即报警
#   bgr, _up = load_image("test_imgs/001.png")       # 或 cv2.imread 等
#   kept, _, _ = extract_contours(cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY),
#                                 rec.p["dilate_k"], rec.p["min_area"])
#   res = rec.recognize(bgr, kept)                   # kept 必传：缺失即报警
#   for s in res["scales"]: print(s["text"], s["value"], s["unit"])
#
# 命令行（单图识别 JSON 摘要；test_imgs 批量分割+识别可视化见 archea_show_rec.py）：
#   python3 archea_rec.py test_imgs/001.png          # 单图，打印 JSON 摘要
#   python3 archea_rec.py --backend paddle 001.png   # 强制指定后端（缺省 auto 等价）
#
# 依赖：cv2 / numpy（几何）；OCR 后端：
#   pip install paddlepaddle paddleocr      # paddle 后端（PP-OCRv6，唯一可用后端）
# ============================================================================ #
"""考古线图轮廓分类识别（archea_seg 下游模块）：调用方传入分割轮廓 kept -> 比例尺(尺体+起点0+x厘米)/序号/文本。"""
...
class OcrBackend:
    ...
    def _init_paddle(self):
        ...
        try:
            self._paddle = TextRecognition(model_name="PP-OCRv6_medium_rec", enable_mkldnn=False)
        except TypeError:
...
# ---------------------------------------------------------------------------
# 命令行：单图识别 JSON 摘要（批量分割/识别可视化见 archea_show_rec.py）
# ---------------------------------------------------------------------------
def _summary_item(res):
    return {
        "backend": res["backend"],
        "image_size": res["image_size"],
        "stats": res["stats"],
        "scales": [{k: sc[k] for k in ("kind", "orientation", "bbox", "bar_bbox",
                                       "text", "raw_text",
                                       "value", "unit", "conf", "verified", "ticks",
                                       "members")}
                   for sc in res["scales"]],
        "serials": [{k: s[k] for k in ("bbox", "text", "no", "conf", "rotated",
                                       "scale_prefix")}
                    for s in res["serials"]],
        "texts": [{k: t[k] for k in ("bbox", "text", "conf", "rotated", "relic")}
                  for t in res["texts"]],
        "others": res["others"],
        "figures": res["figures"],
        "serial_set": res["serial_set"],
    }
...
if __name__ == "__main__":
    ...
    all_text = "--all-text" in args          # 附加开关：恢复文字/字母识别
    args = [a for a in args if a != "--all-text"]
    paths = [a for a in args if os.path.isfile(a)]
    if not paths:
        print("用法: python3 archea_rec.py <image...> [--backend paddle] [--all-text]\n"
              "  打印单图识别 JSON 摘要（CLI 自行先做 extract_contours 分割）。\n"
              "  test_imgs 批量分割+识别可视化请用: python3 archea_show_rec.py")
        sys.exit(1)
    r = ArcheaRec(backend=backend, digits_only=not all_text)
    for pth in paths:
        bgr = imread_cn(pth)
        kept, _removed, _bin = extract_contours(
            cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY),
            r.p["dilate_k"], r.p["min_area"])
        res = r.recognize(bgr, kept)
        print(json.dumps(_summary_item(res), ensure_ascii=False, indent=2))
