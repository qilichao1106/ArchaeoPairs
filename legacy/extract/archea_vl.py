...
# -*- coding: utf-8 -*-
# ============================================================================
# archea_vl.py - 视觉模型(VL)兜底仲裁：考古线图独立器物提取管线。
#
# 重构 (2026-09-03) 目标：
#   1) 解绑单一 VL 后端：任何 OpenAI 兼容的 /chat/completions 接口
#      （DashScope / 火山方舟 / 智谱 BigModel / SiliconFlow / OpenAI /
#       Ollama / 本地 vLLM）均可通过 PRESET 名或环境变量一键切换，
#      无需修改 archea_extract.py。
#   2) 与 archea_extract.py 的器物判别 / 绑定 / 组合逻辑对齐：
#      VlArbiter 的 5 个任务方法与 E1/E2/E3/E4 注释块一一对应
#      （view_rescue_v2 / fragment_check / serial_rescue / group_arb /
#       scale_prefix），调用方继续使用 VlArbiter 即可。
#   3) 调试友好：crop 落盘、prompt/response 落盘、cache/replay、
#      阶段级统计（按 stage 分桶）、verbose 日志、records JSON 导出。
#
# 模块分层（三层）：
#   ┌─ 配置层   PRESETS / VLConfig / load_config（env + .env + preset 名）
#   ├─ 客户端层 VLClient（OpenAI-compatible HTTP + retry + cache + dump_resp
#   │           + 按 stage 分桶统计）
#   └─ 仲裁层   VlArbiter：全部任务逻辑（裁片 / prompt / 解析 / records /
#               单 stage 启停 / 三态兜底）+ archea_extract.py 的全部已用接口
...
#   （prompt 常量与 JSON 解析为模块级数据/工具，不属于独立层）
#
# 向后兼容：
#   - from archea_vl import VlArbiter, ANTI_BIAS_NOTE 仍可用；
#   - v1 判定入口 judge_view_candidates 已删除（2026-09 配方收敛至 v2，
#     见 judge_view_candidates_v2 docstring）；历史对照实验经
#     PROMPT_VIEW_CANDIDATES 常量复现（仅实验存档，生产勿用）；
#   - 默认模型 glm-5.3-flash（火山 Ark）；VL_PRESET=qwen3-vl-plus 切回旧模型。
#
# 一键切换模型（任意一种）：
#   - 命令行：--vl-preset doubao-vision（archea_extract.py 透传）
#   - 环境变量：VL_PRESET=doubao-vision VL_MODEL=... VL_BASE_URL=...
#                VL_API_KEY=...
#   - 代码：VlArbiter(preset="doubao-vision") 或 VlArbiter(config=VLConfig(...))
#
# 调试开关（环境变量 / archea_extract.py CLI 透传）：
...
#
# 用法：
#   from archea_vl import VlArbiter
...
# 兼容 .env 简易加载（存在 python-dotenv 时自动生效；缺失也不致命）
try:
...
_log = logging.getLogger("archea.vl")
...
def _read_env(name: str) -> str | None:
    """env / .env 读取（首选用 dotenv，缺失时退到手工 .env 解析）。"""
    v = os.environ.get(name)
    if v:
        return v
    p = os.environ.get("VL_ENV_FILE", ".env")
...
# ============================================================================
# 仲裁层：VlArbiter（全部任务逻辑 + archea_extract.py 的全部已用接口）
# ============================================================================
# ----- Prompt 模板（模块级常量，便于人工审查/调试/导出）-----
# [实验存档] v1 短版 prompt（生产已弃用，判定入口 judge_view_candidates
# 已于 2026-09 删除）。仅供 _plan_tmp 历史对照实验脚本复现当时条件
# （模型/尺寸/上下文消融均以此 prompt 跑出）；生产配方为下方 V3。
PROMPT_VIEW_CANDIDATES = (
    "以下是考古器物图版中的局部裁剪（按顺序对应 i=0..N-1）。"
    "逐一判断每张裁片的内容：若它是器物的线图视图（可辨认为器物整体/部件/剖面/"
    "纹饰单元的图形），is_view 为 true；若它只是印刷文字、数字、标点、噪点、"
    "污渍或过小的碎片，is_view 为 false。只输出 JSON："
    '{"views":[{"i":0,"is_view":true},{"i":1,"is_view":false}]}'
)
...
class VlArbiter:
    """VL 兜底仲裁器（archea_extract.py 持有实例，调用 5 个任务方法）。

    三层中的仲裁层：全部任务逻辑在此——裁片、prompt 拼装、响应解析、
    records 记录、单 stage 启停、三态兜底。方法与 archea_extract.py 的
    管线触发场景一一对应：
      judge_view_candidates_v2  E1.R1 视图救援 + E1.5 候选漏斗
                                （view_rescue_v2；v1 入口已于 2026-09 删除）
    ...
    def __init__(self, enabled: bool = True,
        ...
        # 统计（保留旧字段名以兼容 archea_extract.py 读取）
    ...
    def _view_flags_chat(self, crops: list, *, stage: str, prompt: str,
        ...
        """分块批量判定 is_view 的共享实现（v1/v2 唯一判定通路）。
    ...
    def judge_view_candidates_v2(self, bgr: np.ndarray, units: list,                                 context: str = "", chunk_size: int = 8
                                 ) -> tuple[list, str]:
    ...
    def dump_records(self, path: str | os.PathLike) -> None:
        """records 列表导出为 JSON（供 arche_extract.py 外部分析/复核）。"""
        out = {
            "config": {
                "preset": self._cfg.preset,
                "model": self._cfg.model,
...
# ============================================================================
# CLI 调试入口（独立运行：python archea_vl.py --preset X --stats ...）
# ============================================================================
def _cli(argv: list | None = None) -> int:
    import argparse
    ap = argparse.ArgumentParser(
        description="archea_vl CLI：模型切换 / 配置自检 / 统计导出")
    ap.add_argument("--preset", default=None,
                    help="VL preset 名（{}）".format("|".join(sorted(PRESETS))))
    ...
    ap.add_argument("--dump-resp", default=None)
    ap.add_argument("--verbose", action="store_true")
    ap.add_argument("--task", default=None,
                    help="stage 启停，例如 'view_rescue_v2:on,group_arb:off'")
    ap.add_argument("--dump-records", default=None,
                    help="导出 records JSON 路径")
    ap.add_argument("--stats", action="store_true", help="打印统计结构")
    ...
    cfg_overrides = {
        k: v for k, v in {
            ...
            "cache_dir": args.cache_dir, "replay_only": args.replay_only,
            "dump_crops_dir": args.dump_crops, "dump_resp_dir": args.dump_resp,
            "verbose": args.verbose, "tasks_enabled": args.task,
        }.items() if v is not None
    }
    vl = VlArbiter(enabled=True, preset=args.preset, **cfg_overrides)
    print(f"[archea_vl] preset={vl.config.preset} model={vl.config.model} "
          f"base_url={vl.config.base_url} "
          f"api_key={'set' if vl.config.api_key else 'MISSING'}")
    print(f"[archea_vl] tasks_enabled={vl.config.tasks_enabled}")
    if args.stats:
        print(json.dumps(vl.stats(), ensure_ascii=False, indent=2))
    if args.dump_records:
        vl.dump_records(args.dump_records)
        print(f"[archea_vl] records dumped to {args.dump_records}")
    return 0


# ============================================================================
# __main__ 自检（image185 同器物判断；真实模型调用）
# ============================================================================
if __name__ == "__main__":
    ...
    try:
        ...
        with open("archea_show_results/image185.json", encoding="utf-8") as fh:
            res = _json.load(fh)
        bgr = cv2.imdecode(np.fromfile("test_imgs/image185.jpg",
                                       dtype=np.uint8), cv2.IMREAD_COLOR)
        W, H = res["image_size"]
        from archea_seg import decode_mask
        from PIL import Image

        def _crop(iid):
            it = next(x for x in res["instances"] if x["id"] == iid)
            b64 = it["mask_base64"] or it["contour_mask_base64"]
            m = np.asarray(decode_mask(b64).resize(
                (W, H), Image.NEAREST), dtype=bool)
            ys, xs = np.nonzero(m)
            x0, x1 = max(0, xs.min()), min(W, xs.max() + 1)
            y0, y1 = max(0, ys.min()), min(H, ys.max() + 1)
            c = np.full((y1 - y0, x1 - x0, 3), 255, np.uint8)
            sub = m[y0:y1, x0:x1]
            c[sub] = bgr[y0:y1, x0:x1][sub]
            return c

        vl = VlArbiter(enabled=True)              # 默认 glm-5.3-flash
        if not vl.config.api_key:
            print("[archea_vl] 未配置 API_KEY（VOLCENGINE_API_KEY / VL_API_KEY），"
                  "无法发起真实调用；退出自检。", file=sys.stderr)
            sys.exit(1)
        # 期望：id7(compact) + id8(profile) 同列同径 -> 大概率同一器物；
        # id7 + id1 几乎肯定不同
        for a, b in [(7, 8), (7, 1), (0, 9)]:
            _crop(a), _crop(b)
            same, reason = vl.same_artifact(
                bgr,
                [int(v) for v in res["instances"][a]["bbox"]],
                [int(v) for v in res["instances"][b]["bbox"]],
                use_full=True)
            print(f"id{a} vs id{b}: same={same} | reason={reason}")
    except FileNotFoundError:
        # 没有 archea_show_results/image185.json 时退到 CLI
        _cli(sys.argv[1:])